...
class Game {
	...
	AIturn() { //AIの思考
		draw(ctx, canvas); //再描画
		var movelist = MovelistFunc(this.board, this.turn); //着手可能な箇所を探索
		if (movelist.length == 0) { //打てるところがない時はパスするか、終了するかを判定する
			var result_Diff;
			[result_Diff, this.turn, this.pass] = isEnd(this.board, this.turn, this.pass); //終了判定
		...
		var AI_name = "AI" + Role[0.5 + this.turn * 0.5]; //AIの名前を定義

		[this.nextmove, this.eval_list] = othello_AI[AI_name](this.board, this.turn); //AIの呼び出し、盤面から最善手及びそれぞれの評価値を返す

		[this.board, this.turn] = putStone(this.board.copy(), this.turn, this.nextmove[0], this.nextmove[1]); //最善手を着手
		draw(ctx, canvas); //再描画
		AIthinkFlag = 0; //AI思考中のロックを解除
	...
	humanTurn(x, y) {
		...
		[this.board, this.turn] = putStone(this.board.copy(), this.turn, x, y);
		draw(ctx, canvas);
...
function draw(ctx, canvas) {
	...
	for (var x = 0; x < BOARDSIZE; x++) {
		for (var y = 0; y < BOARDSIZE; y++) {
			...
			if (game.board[x][y] == 1 || game.board[x][y] == -1) {
				ctx.beginPath();
				if (game.board[x][y] == 1) {
				...
				for (var i = -1; i <= 1; i++) {
					for (var j = -1; j <= 1; j++) {
						[turn_flag, temp_board] = turnStone(game.board.copy(), x, y, i, j, game.turn);
						if (turn_flag == 2) {
...
function analysis() {
	...
	ctx.textAlign = "center";
	ctx.fillStyle = '#ffffff';
	var movelist = MovelistFunc(game.board.copy(), game.turn);
	if (movelist.length == 0) {
		return;
	}
	Role[2] = document.Preference.ana_role.value;
	...
	[nextmove, eval_list] = othello_AI[AI_name](game.board.copy(), game.turn);
...
//----------------------------------------
function upload() {
	var filename = document.Preference.AIname.value;
	var code = othello_AI.AI1.toString();
	code = code.replace(/window\.CP\.exitedLoop\(1\)/g, "");
	code = code.replace(/window\.CP\.exitedLoop\(0\)/g, "");
	code = code.replace(/if \(window\.CP\.shouldStopExecution\(1\)\)\{break;\}/g, "");
